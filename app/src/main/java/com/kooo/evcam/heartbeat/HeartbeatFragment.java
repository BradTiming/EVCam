        ...
        if (getActivity() instanceof MainActivity) {
            ...
            if (isRunning) {
                ...
                if (!screenOnPush && !screenOffPush) {
                    tvStatus.setText("Service not running (all image-upload modes disabled)");
                    tvStatus.setTextColor(0xFFFF9900);  // 橙色
                } else if (screenOffPush && !screenOnPush) {
                    tvStatus.setText("Service not running (screen-off upload only)");
                    tvStatus.setTextColor(0xFFFF9900);  // 橙色
                } else {
            ...
            tvCameraCount.setText("Cameras: " + cameraCount + "/" + totalCameras + " connected");
        }
